...
教训有三条：一是 `level0 corrupted` 只说明序列化数据读取越界，不指向任何具体成因，同一个报错在本项目已经有三个完全不同的来源，每次都必须靠对照实验重新定位；二是排查过程本身会污染环境，下一次构建失败前先确认没有残留的编辑器进程；三是二分要分到"进不进包"这一层，只在组件层面二分会得出错误结论。
