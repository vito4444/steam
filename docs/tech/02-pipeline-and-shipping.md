...
| 组 | 指标 | 材质与几何深化前 | 深化后 | 目标 |
|---|---|---|---|---|
| 构图 | 平均亮度 | 0.1937 | 0.1624 | 0.1484 |
| 构图 | 对比度 | 0.1655 | 0.1413 | 0.1623 |
| 构图 | 过曝占比 | 0.0040 | 0.0044 | 0.0017 |
| 构图 | **细节密度** | 0.0482 | **0.0824** | 0.1947 |
| 构图 | **死黑占比** | 0.1035 | **0.1765** | 0.3997 |
| 构图 | 暗部下限 | 0.0538 | 0.0544 | 0.0000 |

细节密度提升 71%，死黑占比提升 70%，同时平均亮度和对比度仍在容差内——
说明提升来自真实的结构增加，不是靠调亮画面换来的。
...
**同一个报错还有第二个完全不同的成因，值得单独记一笔。** 后来 `level0 corrupted` 又出现过一次，当时的结论是"场景生成与构建必须在同一个编辑器进程内完成"，并据此改了构建脚本。**这个结论是错的。** 真正的原因是前面几轮排查留下了没有退出的 Unity 编辑器进程，多个进程并发读写同一份 `Library` 缓存，产出的构建包自然是坏的。清掉残留进程后，分两个进程反而是稳定的做法，同进程连着做才会出问题——生成期在内存里新建的网格与刚写盘的场景文件之间状态不一致，而 `BuildPlayer` 读的是内存里那一份。

**第四个成因，也是前一个的真正解释：场景内嵌网格的数量。** 补桌面道具时它第四次出现。对照实验定位到道具，但道具本身没有任何异常参数——问题在于给贴图做 UV 平铺的实现会为每个尺寸不同的物件生成一份独立网格，道具让这个数量从三十几涨到六十七。把平铺量量化到 0.25 的整数倍、让网格大量复用之后，数量降回三十二，可玩场景立刻恢复正常。

这多半也解释了第三次：着色器进包并不是因为它自己有问题，而是它增加的包内容压过了那条线。两次都表现为"加了点东西就崩"，而加的东西本身完全正常。

**第三个成因：某些资源只要进入构建包就会触发同一个报错。** 加后处理时它第三次出现。这次与进程残留无关，也与场景组件无关——对照实验很明确：同一个场景挂上后处理组件、但不让着色器进入构建包，运行完全正常；只要着色器以任何形式进包（被场景序列化引用，或者放进 `Resources`），产出的 `level0` 就损坏。而着色器编译过程在日志里没有任何报错，甚至写着 `Serialized binary data for shader Decoder/StationPost in 0.00s`。

根因尚未定位，怀疑与无 GPU 的 batchmode 下着色器变体序列化有关，需要在有显卡的机器上复核。当前的处理是把着色器留在 `Assets/Shaders/` 且不被任何序列化字段引用，因此不进包；编辑器截图路径由 `CaptureHarness` 用 `AssetDatabase` 直接加载并注入，所以画面自检和对外展示仍然是完整成像。

教训有三条：一是 `level0 corrupted` 只说明序列化数据读取越界，不指向任何具体成因，同一个报错在本项目已经出现过四次、追出三种不同的来源，每次都必须靠对照实验重新定位，而且前一次的结论会误导下一次；二是排查过程本身会污染环境，下一次构建失败前先确认没有残留的编辑器进程；三是二分要分到"进不进包"这一层，只在组件层面二分会得出错误结论。

```bash
pgrep -f "Editor/Unity"   # 应当为空
```
...
| 缺口 | 原因 | 影响 |
|---|---|---|
| 细节密度 0.082，参考图 0.195 | 已从 0.048 提升 71%。剩余差距来自部件形态的多样性：参考作品有管道、齿轮、阀门、弹药等大量异形机械件，本项目是规整的面板阵列 | 这是内容量差距而非技术差距，需要更多独特几何体 |
| 死黑占比 0.177，参考图 0.400 | 已从 0.104 提升 70%。剩余差距来自画面构成：参考作品有大面积处于背光的舱壁结构 | 需要在视野边缘增加遮挡体 |
| 配色未达自定档案 | 自动配平陷入局部最优，突破需要空间上分离光源作用域而非继续调强度 | 冷暖分区已改善，仍偏冷 |
...
| 后处理只在编辑器截图路径生效 | 着色器进入构建包即导致 level0 损坏，根因未定位 | 玩家实际运行时没有渐晕、颗粒、色差与扫描线，需在有显卡的机器上复核 |
