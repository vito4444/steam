...
v019 把体积、屋顶和作物补上之后，把两张图并排缩到同一宽度再看，剩下的差距不在「少画了什么」，而在三件更基础的事上：画面本身不是逐像素对齐的，所有散落物都精确压在格心上，以及每一处地面类型的交界都是一刀切的菱形。这一轮处理的主要是这三件，外加地下层第一次被当作一个要看的画面来做。

![对照：上为概念稿，下为当前](progress/v020-side-by-side.png)

### 一、相机不是整数缩放

相机一直是按聚落的实际范围算出来的，算出多少就用多少，结果落在每个美术像素对应约 1.65 个屏幕像素的位置上。点采样在非整数倍下仍然保持硬边，但边是不均匀的：贴图的某几行占两个屏幕像素、相邻几行占一个，于是一道笔直的檐口带上了波纹，地面的抖动噪点会在镜头移动时爬行。这是「一帧像素画」和「一张像素画的照片」之间的区别。

缩放现在只允许整数档，滚轮在一到三档之间跳，开局停在二档。

**选二档而不是一档是量出来的，不是拍脑袋定的**：把参照物按同一宽度放在截图旁边，参照物一屏覆盖约二十五格，一档覆盖三十格，二档覆盖十五格。单看格数一档更接近，但格数量的是错误的东西——一档下聚落是一片空牧场中间的孤岛，门洞、院子里的货物和人全都小到读不出来；二档下镇子填满画幅并从四边跑出去，而这正是参照物的构图。玩家可以退到一档看全局。

这一档也把之前「按范围取景」的整套逻辑作废了：`verticalCrop`、`fill` 这些为了硬凑构图而设的系数全部删掉，相机只负责对准镇子中心。

### 二、每一棵树都站在格心上

野外的树、灌木、石头原来都精确落在格子中心，于是整张地图明晃晃地摆在一个点阵上：一片林子看起来像果园，散落的石头看起来像摆出来的。散落物现在按格子坐标的散列偏离中心，横向偏移量是纵向的两倍——一格宽是高的两倍，等量的世界偏移在横向上读起来会大得多。

栅栏、河岸、栈桥不参与抖动：它们属于格子的边，必须和邻格对得上。

### 三、地面交界

两种纯色沿格子边界相接，画出来的就是那条边界；满地图都是这样的边界，画出来的就是整张网格。之前的镇子看着像贴瓷砖，每个院子都是压进草地里的一枚干净菱形。

现在交界的两侧各自往对面撒自己的边缘：泥土被鞋和车轮带到草地上，草从院子边缘长进来又被踩回去。两边一叠，接缝有了几个像素的宽度，而且沿长度方向处处不同。踩秃一侧的草撒得比另一侧稀——在用的院子里草只剩几根，按同样密度撒会变成围着每块地画了一圈绿边。

这一层纯粹是撒点，模拟层对格子的判断没有任何变化。

同一轮里裸土本身也补了纹理。原来它只有逐像素抖动，在二档缩放下就是一整张平板，而院子占画面的比例相当大。现在上面有沿网格轴的拖痕、被车轮带上来的碎石、踩进土里的麦秸；巷子中间被磨亮的路拱和两侧甩出的碎石也一并补上。

### 四、河岸

深水直接贴着旱地沿菱形边收口，河读起来是「一块盖在地图上的蓝色」而不是「地图里的水」，而沿边界那条浪花线是在替网格描边。

河岸现在向水里退晕出一条浅水带，深浅随格子变化，所以它的内缘不跟着任何规则的东西走。浪花打碎成点，落在离岸一两个像素的地方。

礁石同时减量：带石头的河岸变体从八分之五降到八分之三，石头更小，颜色转棕。冷灰的石头在这套色板里读作混凝土，而且会把视线拉到河岸上——那里只是布景。

### 五、人

村民原来是从靴子到肩膀一整块外套色的方柱加一个头，在这个尺寸下读作板条箱。现在按部件搭：两条腿中间留缝、下摆张开到大腿中部的束腰长衫、腰带、挂在身体外侧末端带手的胳膊，以及一个更小的、受光方向与全场一致的头。

### 六、作物、麦垛与烟囱

麦田是两条交替的色带，读作地毯。现在是条播的行：行与行之间露出裸土，立着的麦子纹理是碎的，偶尔有一穗高出行外。

麦垛原来是均匀网格上的六个一模一样的圆锥，像一排帐篷。现在离开那个网格，高度各不相同，腰上有捆绳，底下参差不齐。

烟囱原来是九像素宽的一整块单色，是每栋农舍上最重的一块。现在窄了，砌出砖层，沿转角分成受光面和背光面。

### 七、草与土分不开

上一轮照着参照物调橄榄色，把草皮和裸土调到了每个通道都相差十来个数值的地步。远看整个聚落化成一整片黄褐，里面读不出地块、路肩和巷子。参照物的草确实是那个橄榄色，但它衬的是屋顶和阴影，不是成片的踩秃地面；在这里承担对比的是地面本身。

草现在落在红绿之间的中点，裸土保留偏红，巷子比它们都亮一档。这一改，地块结构立刻能读出来了。

### 八、院子里空无一物

作业院子之前是扫干净的。杂物密度提高，每件带变体（两堆柴不再是同一堆柴），品类按「一个在用的院子里实际有多少」加权。新增三种：干草垛、码起来阴干的砖、插着斧头的劈柴墩。

干草垛第一版是平的黄三角——只有最边上一列是暗的。现在明暗过渡横跨整个面，外面绕着一圈圈草绳，顶上留着当初堆垛用的杆子。

### 九、地下层

这是地下层第一次按「玩家要看的画面」来做。

![地下](progress/v020-underground.png)

实心土原来是平棕色上五个暗点，而玩家一进地下，它就是屏幕上的绝大部分——所有坑道都要衬在它上面读。现在它有不同土色的条带、挖掘要绕开的石块，以及浅到还活着的树根。

挖开的格子原来在自己的边缘一圈压暗，理由是「一段坑道应该读作有壁的走廊」。但一间厅堂中间的格子根本没有壁，这一压就把网格画满了整片工事。壁脚的阴影现在由渲染层按边界投，它知道哪几侧后面真的还有岩石：厅堂读作一整片被暗边框住的亮地面，坑道仍然读作走廊。

顺带修了两个 bug：

**未申报的密室原本挖在镇政厅底下。** 开局日志写的是「酒坊底下有一间未申报的地窖」，而竖井是按街心的固定偏移放的。上一轮为了不让磨坊塔顶到小地图后面，把酒坊和镇政厅在 civic 排上对调，竖井没跟着走，于是全镇唯一一处见不得人的工事跑到了镇政厅底下——那是唯一一栋地窖不需要向任何人解释的建筑，也不是日志或任何按常理推断的玩家会去找的地方。现在竖井取自酒坊实际落位（放置失败时会向外螺旋找空地，所以不能用请求的坐标）。

**截图工具够不到地下层。** 切层那段代码被包在跑预热的分支里，单独要求深度会安静地拍成地表，只有同时要求快进才能拍到地窖。

### 地表

![地表](progress/v020-surface.png)

### 仍未追平的差距

- **地形起伏**。参照物有高差、护坡、石砌水岸。当前地面完全是平的；这需要给地图加高程，是模拟层的改动。黏土坑的台阶仍是这张图上唯一的真实高差，而它是画出来的，不是地形。
- **建筑平面**。偏厦、门廊、八种农舍变体、两种非茅草屋顶、磨坊塔、横向山墙都有了，主体仍是矩形。真正的 L 形平面要动 footprint，那是模拟层。
- **人的数量**。参照物里同屏十几个人在干活，这里六个。人口直接进经济账：这一轮试过把住宅从六栋加到十六栋来填满画面，结果应税建筑数上升、税金从黑钱里扣了 60，`ContrabandSellsForFarMoreThanAleAndNeverReachesTheBooks` 直接变红。已经回退——为画面改经济不划算，而改测试断言去迁就它更不行。
- **建筑相对画幅的尺寸**。二档下我们的建筑比参照物大约四成，一档下小约四成，中间没有整数档。要真正对上，得把美术按更小的格子重画（参照物大概率是小格美术放大显示的），那是整套美术资产的改动。
- **屋顶与墙面的细节密度**。二档缩放把它们放大之后，大面积的屋顶和墙比参照物素。茅草这一轮补了顺坡的草束纹理，墙面还没动。
...
生成的河在地图中间一半的范围里蜿蜒而下，多数种子下它落在取景之外，为它画的所有河岸、浅滩和礁石都没人看得到。镇子的东西位置现在由河决定，站在离水十四格的地方。

这个距离既是取景需要，也是模拟层的约束：河床下面是地下水，挖进去的工事会被淹，紧贴河岸建的镇子就是地窖会进水的镇子。十四格让河刚好在视野边缘，同时仍然远在任何人挖掘的范围之外。

河一进画面就暴露了问题——一整片橄榄与棕色里横着一条亮青色、中间还有白色浪头，是全图唯一看起来像别的游戏的东西。水、浪花和岸边的黏土都压到了其余色板能与之并列的程度。

河边立了两处栈桥：铺板的台面、打进河床的木桩、台上留着一只桶。任何河边聚落都有一处是特意用来靠近水的，没有它，河就只是从一个用不上它的镇子旁边流过的布景——而这条河旁边是酒坊。

栈桥是**挑出来的，不是撒出来的**。按十一分之一的概率在河岸上撒，取景之内一个也没落到：河道一旦开始摆动，恰好只有一侧临陆的格子就只占河岸的一小部分，而一个可能存在也可能不存在的特征不算特征。现在取距镇子最近的两处合格河岸，彼此相隔六格。单侧临陆这个条件把它们挡在河湾和沙嘴之外——建在河湾里的栈桥有半个台面埋在岸里。

### 五、磨坊塔

参照物有风车和水车，两者都比周围任何东西高，形状也都不是房子。这里最高的一直是镇政厅的钟楼小塔，而一座天际线全是屋脊的聚落，读起来就是一种建筑重复了很多遍。

磨坊也是这个镇子唯一不需要为它编一门生意的地标——酒坊本来就得磨麦芽。塔立在酒坊西山墙外，收分向上（这一点把它和烟囱区分开），锥形帽顶，四片叶按斜十字装（正十字读作脚手架，而停转的磨坊本来也多半把叶片停在对角位）。

模拟层没有任何改动。酒坊的配方、造价、产出全都照旧，只是同一栋建筑把它做的事写在了外面。

酒坊和镇政厅在开局布局里对调了。civic 那一排的东端投影到屏幕右上角，正好在小地图后面，塔立起来就顶进去了；钟楼矮，放得下。两者到仓库的距离一样，只是屏幕位置换了。

### 六、视距

把聚落刚好装进画幅（哪怕带裁切），1920 像素宽的画面里横着约三十格；参照物里接近二十格，而这个差别正是它的建筑显得厚实、我们的看起来像桌上模型的主要原因。现在镇子在东西两侧跑出画外——参照物也是这样，它是一处地方的一个视角，不是这处地方的全身像。
...
画面均值从 (96, 85, 44) 降到 (92, 76, 43)，目标是 (75, 64, 37)。这一轮压的是巷子、院子和作物——它们合起来占的面积比建筑还大，而巷子当时在 (150, 118, 70)，参照物的巷子接近 `#69521d`。草地同时把红绿分开了：参照物采样的草在 `#5f5218` 到 `#785c2e` 之间，红高于绿，我们原本接近中性。

### 仍未追平的差距

- **地形起伏**。参照物有高差、护坡、石砌水岸。当前地面完全是平的；这需要给地图加高程，是模拟层的改动。黏土坑的台阶是第一次在这张图上出现真实高差，但它是画出来的，不是地形。
- **建筑平面**。有了偏厦、门廊、八种农舍变体、两种非茅草屋顶、一座磨坊塔和横向山墙（三种变体带），但主体仍是矩形。参照物里几乎没有两栋建筑是同一个平面。横向山墙改的是屋顶轮廓，不改占地——真正的 L 形平面要动 footprint，那是模拟层。
- **人的数量**。参照物里同屏十几个人在干活。这里只有六个村民，而人口直接进经济账，不能为了画面加人。
- **镇子在画幅里的占比**（已基本解决）。等距投影下任何矩形格子区域的屏幕宽高比恒为 2:1，与该区域自身长宽比无关，所以「刚好装下」时 16:9 画幅两侧必然留白。允许东西两侧跑出画外之后这个约束不再决定构图，代价是玩家需要滚动才能看到镇子边缘。
